...
#시작 시간
start_ticks = pygame.time.get_ticks() # 현재 시간을  tick을 받아둠
...
while running:
    ...
    # 타이머 경과 시간계산
    elapsed_time = (pygame.time.get_ticks()- start_ticks) / 1000 # 경과 시간(ms)을 1000으로 나누어서 초(s) 단위로 설정
    timer = game_font.render(str(int(total_time-elapsed_time)),True,(255,255,255)) # 정수형 인수를 통해 int형으로 함
    # render는 폰트를 그리는 형식이며, 출력할 글자(시간정보), antialias는 그냥 true, 글자 색상을 정해서 그린다.
...
# 게임 종료 직전의 대기 시간
pygame.time.delay(2000) # 초 단위이기에 2000이면 2초 정도 대기 할수 있다.
